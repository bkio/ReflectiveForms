...
public static class RfBuilder
{
    public static RfConfigurationBuilder Build(ILogger logger)
    {
        ...
        //
        // Set CROSSCLOUDKIT_DISABLE_BUNDLED_LLM=true to skip ALL bundled LLM
        // model loading. AI features on entities are auto-disabled when
        // AiServiceConfiguration is null. Use this for E2E test runs to avoid
        // out-of-memory crashes from GGUF model loading on CPU.
        ...
        if (!disableBundledLlm)
        {
            ...
            aiConfig = new AiServiceConfiguration(
