...
public sealed class Url : Field
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (casted.Length > 0 && !NetworkUtilities.IsValidUrl(casted))
        {
            return Task.FromResult(OperationResult<bool>.Failure($"Field {jNeedleFieldName}: Should represent a valid URL.", HttpStatusCode.BadRequest));
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        element.InnerHtml = $"<a href='{asString}'>{asString}</a>";
